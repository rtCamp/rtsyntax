...
class rtSyntax {
	...
	/**
	 * Render Admin Option Page With Options Menu
	 */
	...
	/**
	 * @param $content String Modify The Content
	 *
	 * @return string
	 */
	...
	/**
	 * Enqueue The Required StyleSheet
	 */
	...
	/**
	 * Load And Initilize The highlight.js Object
	 */
	...
	/**
	 * Register And Enqueue Required Style, Js For Gutenberg rtSyntax Blocks
	 */
	public function register_rtsyntax_block() {
		// Enqueue Theme css
		$options = get_option( 'rtsyntax_options' );
		wp_enqueue_style( 'rtsyntax-' . $options['theme'], plugin_dir_url( __FILE__ ) . 'css/' . $options['theme'] . '.css' );
		wp_enqueue_style( 'rtsyntax-css-' . $options['theme'], plugin_dir_url( __FILE__ ) . 'blocks/syntax-highlighter/css/' . $options['theme'] . '.css' );
		wp_enqueue_style( 'rtsyntax-editor-css' . $options['theme'], plugin_dir_url( __FILE__ ) . 'blocks/syntax-highlighter/css/editor.css' );
		// Enqueue the required js
		wp_enqueue_script(
			'highlights-js',
			plugins_url( 'blocks/syntax-highlighter/js/highlight.min.js', __FILE__ )
		);
		// Register Our build js file
		wp_register_script(
			'rtsyntax-gutenberg-blocks',
			plugins_url( 'blocks/syntax-highlighter/build/block.build.js', __FILE__ ),
        ...
        wp_localize_script(
                ...
                'rtsyntax_localize',
                array(
                        'is_debug_mode' => ( WP_DEBUG ) ? "true" : "false",
                )
        );
		// register our rtsyntaxdemo/syntaxhighlighter block
		register_block_type(
			'rtsyntax/syntaxhighlighter', array(
				'script' => [ 'highlights-js', 'rtsyntax-gutenberg-blocks', 'rtsyntax-localize-script' ],
				'style'  => 'rtsyntax-' . $options['theme'],
			)
		);
	}

	/**
	 * Register Setings For Theme Support
	 */
	...
	/**
	 * Add Theme Options In Settings Field
	 */
